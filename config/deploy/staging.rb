...
set :project, 'hack15' # the GitHub project name
set :github_user, "mergeweb" # Your GitHub username
set :domain_name, "hack15.upupdev.net" # should be something like project.mydomain.com
...
set :branch, $1 if `git branch master`  # This is for a git branch if you want to deploy from something besides master
...
after "deploy:update_code" do
  run "ln -s #{shared_path}/webroot/cache_css #{release_path}/webroot"
  run "ln -s #{shared_path}/tmp/cache #{release_path}/tmp"
  run "ln -s #{shared_path}/config/compass/sass #{release_path}/config/compass"
  run "ln -s #{shared_path}/logs #{release_path}"
  run "ln -s #{shared_path}/tmp/sessions #{release_path}/tmp"
  run "ln -s #{shared_path}/tmp/tests #{release_path}/tmp"
  run "ln -s #{shared_path}/config/app.php #{release_path}/config"
  run "chmod -R 755 #{release_path}"
  run "composer update -d #{release_path} --no-dev"
end
after "deploy" do
  run "rm -rf #{shared_path}/tmp/cache/models/*"
  run "rm -rf #{shared_path}/tmp/cache/persistent/*"
  run "#{release_path}/bin/cake AssetCompress.AssetCompress build"
end
